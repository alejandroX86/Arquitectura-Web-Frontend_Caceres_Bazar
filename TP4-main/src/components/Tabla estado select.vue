<template>
  <div>
    <div>N° ID Estado:
      <select v-model="idu" @change="cambiarDescripcionE">
...
onMounted(async () => {
  ...
  const responseAsignacionS = await fetch('http://localhost:3000/estados', options);
  BaseS.value = await responseAsignacionS.json();
  BaseE.value = Base.value;
});

const cambiarDescripcionE = async () => {
  const options = {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
  ...
  const responseAsignacion = await fetch(`http://localhost:3000/asignar/estado/` + idu.value, options);
